...
def register_and_validation(base_url, e_phone, e_name):
    """註冊商家 > 通過審核"""
    worky_101.e_register(base_url, e_phone)
    worky_102.e_register_confirm(base_url, e_phone)
    worky_105_1.e_shop_create(base_url, e_name)
...
def repeat_register_and_validation(base_url, time, e_phone, e_name):
    """註冊(多個)商家"""
    e_phone = int(e_phone)
    time = int(time)
    ...
    try:
        ...
        for i in range(time):
            ...
            register_and_validation(base_url, e_phone, e_name)
            e_phone += 1
            name_prefix = "".join([c for c in e_name if not c.isdigit()])  # 提取非數字部分
            name_number = "".join([c for c in e_name if c.isdigit()])      # 提取數字部分
            e_name = f"{name_prefix}{int(name_number or '0') + 1}"         # 數字部分加 1 (為空時回傳 '0')
